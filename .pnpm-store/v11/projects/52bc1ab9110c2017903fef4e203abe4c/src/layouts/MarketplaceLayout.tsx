...
interface MarketplacePartnerDealer {
  companyName: string;
  logoUrl?: string | null;
...
export function MarketplaceLayout() {
  ...
  const [partnerDealers, setPartnerDealers] = useState<MarketplacePartnerDealer[]>([]);
  ...
  const stores = useMemo(() => (
    (marketplace?.stores || [])
      ...
      .map((store) => ({
        ...
        bannerImages: (store.bannerImages || []).map((image) => ({
          ...image,
          imageUrl: resolveApiUrl(image.imageUrl),
        })),
        price: store.price,
  ...
  useEffect(() => {
    ...
    let active = true;
    apiClient
      .get<MarketplacePartnerDealer[]>(`/api/public/dealers/marketplaces/${encodeURIComponent(bankSlug)}`)
      .then((response) => {
        if (active) setPartnerDealers(response.data || []);
      })
      .catch((error) => {
        console.error('Failed to load marketplace partner dealers:', error);
        if (active) setPartnerDealers([]);
      });
  ...
  return (
    ...
    >
      ...
      {isChatbotEnabledForActiveStore && (
        <Chatbot
          storeName={activeStore?.label || activeStore?.name}
      ...
      <footer className="bg-[linear-gradient(180deg,#0f172a_0%,#10192d_100%)] text-white py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-10 md:grid-cols-[1.3fr_1fr_1fr_1fr]">
            ...
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-300">Concessionnaires</h3>
              <div className="mt-4 space-y-3 text-sm text-slate-400">
                {partnerDealers.length > 0 ? partnerDealers.map((dealer) => {
                  const dealerLogo = resolveApiUrl(dealer.logoUrl);
                  return (
                    <div key={`${dealer.companyName}-${dealer.storeName || ''}`} className="flex items-center gap-2.5">
                      {dealerLogo ? (
                        <img src={dealerLogo} alt="" className="h-7 w-7 rounded-md border border-white/10 bg-white object-contain p-0.5" />
                      ) : (
                        <span className="flex h-7 w-7 items-center justify-center rounded-md bg-white/10 text-slate-300">
                          <Building2 className="h-3.5 w-3.5" />
                        </span>
                      )}
                      <span className="min-w-0">
                        <span className="block truncate transition-colors hover:text-white">{dealer.companyName}</span>
                        {dealer.storeName && <span className="block truncate text-xs text-slate-500">{dealer.storeName}</span>}
                      </span>
                    </div>
                  );
                }) : (
                  <p>Aucun concessionnaire partenaire.</p>
                )}
              </div>
            </div>
          </div>
