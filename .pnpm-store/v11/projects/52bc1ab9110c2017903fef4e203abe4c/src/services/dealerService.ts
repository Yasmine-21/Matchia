...
export type PartnershipContractStatus = 'DRAFT' | 'PENDING_ACCEPTANCE' | 'ACTIVE' | 'SUPERSEDED' | 'EXPIRED' | 'TERMINATED' | 'CANCELLED';
...
export interface DealerProductDocument { id: number; documentType: string; fileName: string; url: string; publicDocument: boolean; uploadedAt?: string }
export interface DealerProductCatalogImage { id: number; imageUrl: string; displayOrder: number }
export interface DealerProduct {
  ...
  parameterValues: ParameterValue[]; documents: DealerProductDocument[]; catalogImages: DealerProductCatalogImage[]; createdAt?: string; updatedAt?: string;
}
...
export interface DealerFinancingRequest {
  id: number; reference: string; productId: number; productName: string; productImageUrl?: string; productPrice?: number;
  clientName: string; clientEmail?: string; clientPhone?: string; bankId: number; bankName: string; bankLogoUrl?: string;
  storeId: number; storeName: string; requestedAmount?: number; monthlyPayment?: number; downPayment?: number;
  durationMonths?: number; status: 'DRAFT' | 'PENDING' | 'ACCEPTED' | 'REJECTED'; createdAt: string; processedAt?: string;
}
...
export interface PartnershipContract {
  id: number; contractNumber: string; versionNumber: number; templateVersion: string; partnershipId: number; dealerId: number; dealerName: string;
  ...
  contractTerms: string; terminationConditions: string; specificConditions?: string; generatedDocumentPath?: string; dealerAcceptedAt?: string; bankAcceptedAt?: string;
...
export interface PartnershipContractPayload {
  ...
  contractTerms?: string; terminationConditions?: string; specificConditions?: string;
}
export interface PartnershipContractPreview { html: string; contractNumber: string; versionNumber: number; finalized: boolean }
...
export const dealerService = {
  ...
  createContractRevision(partnershipId: number) { return apiClient.post<PartnershipContract>(`/api/bank/dealers/partnerships/${partnershipId}/contract/revisions`); },
  bankContractPreview(id: number) { return apiClient.get<PartnershipContractPreview>(`/api/bank/dealers/contracts/${id}/preview`); },
  downloadBankContract(id: number) { return apiClient.get<Blob>(`/api/bank/dealers/contracts/${id}/download`, { responseType: 'blob' }); },
  ...
  dealerContractPreview(id: number) { return apiClient.get<PartnershipContractPreview>(`/api/dealer/contracts/${id}/preview`); },
  downloadDealerContract(id: number) { return apiClient.get<Blob>(`/api/dealer/contracts/${id}/download`, { responseType: 'blob' }); },
  ...
  saveProduct(data: Record<string, unknown>, image?: File, id?: number, catalogImages: File[] = [], catalogImageIdsToDelete: number[] = []) {
    const form = new FormData(); form.append('data', jsonPart(data)); if (image) form.append('image', image);
    catalogImages.forEach((catalogImage) => form.append('catalogImages', catalogImage));
    catalogImageIdsToDelete.forEach((catalogImageId) => form.append('catalogImageIdsToDelete', String(catalogImageId)));
  ...
  uploadProductDocument(productId: number, file: File, documentType: string, publicDocument: boolean) {
    const form = new FormData(); form.append('file', file); form.append('documentType', documentType); form.append('publicDocument', String(publicDocument));
    return apiClient.post<DealerProductDocument>(`/api/dealer/products/${productId}/documents`, form);
  },
  deleteProductDocument(productId: number, documentId: number) { return apiClient.delete(`/api/dealer/products/${productId}/documents/${documentId}`); },
  uploadProductCatalogImages(productId: number, files: File[]) {
    const form = new FormData(); files.forEach((file) => form.append('files', file));
    return apiClient.post<DealerProductCatalogImage[]>(`/api/dealer/products/${productId}/catalog-images`, form);
  },
  deleteProductCatalogImage(productId: number, imageId: number) {
    return apiClient.delete<DealerProduct>(`/api/dealer/products/${productId}/catalog-images/${imageId}`);
  },
  getProductDocument(productId: number, documentId: number) { return apiClient.get<Blob>(`/api/dealer/products/${productId}/documents/${documentId}/download`, { responseType: 'blob' }); },
  ...
  financingRequests(params?: { bankId?: number; storeId?: number; productId?: number; status?: string; search?: string }) { return apiClient.get<DealerFinancingRequest[]>('/api/dealer/financing-requests', { params }); },
  financingRequest(id: number) { return apiClient.get<DealerFinancingRequest>(`/api/dealer/financing-requests/${id}`); },
