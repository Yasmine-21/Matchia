...
import { FileSignature, Send, ShieldCheck, Eye, GitBranch } from 'lucide-react';
...
const labels: Record<PartnershipContract['status'], string> = {
  ...
  SUPERSEDED: 'Version remplacée', EXPIRED: 'Expire', TERMINATED: 'Resilie', CANCELLED: 'Refuse',
};
...
export function BankContractsPanel({ contracts, onChanged }: { contracts: PartnershipContract[]; onChanged: () => Promise<void> }) {
  ...
  const [previewing, setPreviewing] = useState<PartnershipContract | null>(null);
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: PartnershipContractPayload = {
      ...
      specificConditions: String(data.get('specificConditions') || ''),
    };
    setBusy('save');
    try {
  ...
  return <>
      ...
      {contracts.map((contract) => (
        <Card key={contract.id} className="flex h-full flex-col p-6">
          ...
          <div className="mt-auto flex flex-wrap gap-3 pt-5">
            <Button variant="outline" icon={<Eye className="h-4 w-4" />} onClick={() => setPreviewing(contract)}>Aperçu</Button>
            ...
            {(contract.status === 'ACTIVE' || contract.status === 'EXPIRED' || contract.status === 'TERMINATED') && <Button variant="secondary" icon={<GitBranch className="h-4 w-4" />} loading={busy === `revision-${contract.id}`} onClick={() => void action(`revision-${contract.id}`, () => dealerService.createContractRevision(contract.partnershipId), 'Nouvelle version brouillon créée.')}>Nouvelle version</Button>}
          </div>
        </Card>
      ))}
    </div>
    ...
    <ContractPreviewModal contract={previewing} side="bank" onClose={() => setPreviewing(null)} />
  </>;
}
...
function ContractForm({ contract, busy, onSubmit, onCancel }: { contract: PartnershipContract; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void }) {
  ...
  return <form onSubmit={onSubmit} className="space-y-5">
    ...
    <Field label="Conditions particulières"><textarea name="specificConditions" rows={3} defaultValue={contract.specificConditions || ''} placeholder="Clauses spécifiques facultatives" className="w-full rounded-lg border border-input bg-input-background px-3 py-3 outline-none focus:ring-2 focus:ring-ring" /></Field>
