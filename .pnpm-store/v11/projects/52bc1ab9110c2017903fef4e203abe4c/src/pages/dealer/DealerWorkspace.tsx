...
import { getStoreParameterInput, getStoreProductFamily, storeProductFamilyLabel } from '../../config/storeProductSchema';
...
export function DealerWorkspace({ mode }: { mode: Mode }) {
  ...
  const [productDetails, setProductDetails] = useState<DealerProduct | null>(null);
  ...
  const saveProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const catalogImageFiles = form.getAll('catalogImages').filter((entry): entry is File => entry instanceof File && entry.size > 0);
    const catalogImageIdsToDelete = form.getAll('catalogImageIdsToDelete')
      .map((entry) => Number(entry)).filter((id) => Number.isInteger(id) && id > 0);
    const documentFiles = form.getAll('documents').filter((entry): entry is File => entry instanceof File && entry.size > 0);
    ...
    try {
      const saved = await dealerService.saveProduct({
        ...
        parameterValues,
      }, image, editProduct?.id, catalogImageFiles, catalogImageIdsToDelete);
      await Promise.all([
        ...documentFiles.map((file) => dealerService.uploadProductDocument(
          saved.data.id,
          file,
          String(form.get('documentType') || 'Document produit'),
          form.get('publicDocument') === 'on',
        )),
      ]);
  ...
  const deleteProductDocument = async (productId: number, documentId: number) => {
    try {
      await dealerService.deleteProductDocument(productId, documentId);
      setEditProduct((current) => current?.id === productId
  ...
  const downloadProductDocument = async (productId: number, documentId: number, fileName: string) => {
    try {
      const response = await dealerService.getProductDocument(productId, documentId);
      const url = URL.createObjectURL(response.data);
      const link = document.createElement('a'); link.href = url; link.download = fileName; link.click(); URL.revokeObjectURL(url);
    } catch (error) {
      toast.error(getErrorMessage(error, 'Impossible de télécharger le document.'));
    }
  };
  ...
  const addStock = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isInteger(quantity) || quantity <= 0) {
      toast.error('Saisissez une quantité entière supérieure à zéro.');
      return;
    }
    setAddingStock(true);
    try {
      await dealerService.addProductStock(productForStock.id, quantity);
  ...
  return (
    <div className="space-y-6">
      ...
      <ProductFormModal
        ...
        onDeleteDocument={deleteProductDocument}
        onDownloadDocument={downloadProductDocument}
        onClose={() => { if (!savingProduct) { setShowProductForm(false); setEditProduct(null); } }}
        onSubmit={saveProduct}
      />

      <ProductDetailsModal product={productDetails} onClose={() => setProductDetails(null)} />

      <Modal isOpen={Boolean(productForStock)} onClose={() => { if (!addingStock) setProductForStock(null); }} title="Ajouter du stock" size="sm">
        <form className="space-y-5" onSubmit={addStock}>
          <div className="rounded-2xl border border-primary/15 bg-primary/5 p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-primary">Produit sélectionné</p>
            <p className="mt-1 text-lg font-semibold text-foreground">{productForStock?.name}</p>
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div><span className="block text-muted-foreground">Disponible</span><b>{productForStock?.availableStock ?? '-'}</b></div>
              <div><span className="block text-muted-foreground">Réservé</span><b>{productForStock?.reservedStock ?? '-'}</b></div>
            </div>
          </div>
          <label className="block space-y-2 text-sm font-medium text-foreground">
            Quantité à ajouter
            <input autoFocus type="number" min="1" step="1" value={stockQuantity} onChange={(event) => setStockQuantity(event.target.value)} className="h-12 w-full rounded-lg border border-input bg-input-background px-3 text-base outline-none focus:ring-2 focus:ring-primary/40" />
          </label>
          <p className="text-sm text-muted-foreground">Le stock total et disponible seront augmentés. Les unités déjà réservées ne seront pas modifiées.</p>
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <Button type="button" variant="outline" disabled={addingStock} onClick={() => setProductForStock(null)}>Annuler</Button>
            <Button type="submit" loading={addingStock} icon={<Plus className="h-4 w-4" />}>Ajouter au stock</Button>
          </div>
        </form>
      </Modal>
                    ...
                    <tr key={product.id} className="hover:bg-muted/20">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" icon={<Eye className="h-4 w-4" />} onClick={() => onView(product)}>Détails</Button>
          ...
          {products.map((product) => {
            ...
            <article key={product.id} className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all hover:border-primary/30 hover:shadow-md">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                {product.catalogImages?.length > 0 && (
                  <div className="mt-4 flex items-center gap-2" aria-label={`${product.catalogImages.length} images dans le catalogue`}>
                    <div className="flex -space-x-2">
                      {product.catalogImages.slice(0, 4).map((image) => (
                        <img key={image.id} src={resolveApiUrl(image.imageUrl)} alt="" className="h-9 w-9 rounded-lg border-2 border-card bg-muted object-cover" />
                      ))}
                    </div>
                    <span className="text-xs font-medium text-muted-foreground">{product.catalogImages.length} image{product.catalogImages.length > 1 ? 's' : ''} catalogue</span>
                  </div>
                )}
...
function ProductDetailsModal({ product, onClose }: { product: DealerProduct | null; onClose: () => void }) {
  const [selectedImageUrl, setSelectedImageUrl] = useState<string | undefined>();
  const mainImageUrl = product?.imageUrl ? resolveApiUrl(product.imageUrl) : undefined;
  const catalogImages = product?.catalogImages || [];

  useEffect(() => { setSelectedImageUrl(mainImageUrl); }, [product?.id, mainImageUrl]);

  return (
    <Modal isOpen={Boolean(product)} onClose={onClose} title="Détails du produit" size="lg">
      {product && (
        <div className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(260px,0.85fr)]">
            <div className="flex min-h-80 items-center justify-center overflow-hidden rounded-2xl border border-border bg-muted/30 p-5">
              {selectedImageUrl ? <img src={selectedImageUrl} alt={product.name} className="max-h-[420px] w-full object-contain" /> : <ImageIcon className="h-14 w-14 text-muted-foreground" />}
            </div>
            <div className="space-y-4">
              <div><Badge variant={statusMeta(product.status).variant}>{statusMeta(product.status).label}</Badge><h3 className="mt-3 text-2xl font-bold text-foreground">{product.name}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{product.description || 'Aucune description fournie.'}</p></div>
              <div className="rounded-xl bg-muted/45 p-4"><span className="text-xs uppercase tracking-wider text-muted-foreground">Prix</span><p className="mt-1 text-xl font-bold text-destructive">{formatTnd(Number(product.price))}</p></div>
              <div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg border border-border p-2"><b className="block text-base">{product.totalStock ?? '-'}</b>Total</div><div className="rounded-lg border border-border p-2"><b className="block text-base">{product.availableStock ?? '-'}</b>Disponible</div><div className="rounded-lg border border-border p-2"><b className="block text-base">{product.reservedStock ?? '-'}</b>Réservé</div></div>
            </div>
          </div>
          <section className="rounded-2xl border border-border bg-muted/10 p-5">
            <div className="flex items-center justify-between gap-3"><div><h4 className="font-semibold text-foreground">Images du catalogue</h4><p className="mt-1 text-sm text-muted-foreground">Sélectionnez une image pour l’afficher en grand.</p></div><Badge variant="secondary">{catalogImages.length} image{catalogImages.length > 1 ? 's' : ''}</Badge></div>
            {catalogImages.length > 0 ? (
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {catalogImages.map((image) => {
                  const imageUrl = resolveApiUrl(image.imageUrl);
                  const isSelected = imageUrl === selectedImageUrl;
                  return <button key={image.id} type="button" onClick={() => setSelectedImageUrl(imageUrl)} className={`overflow-hidden rounded-xl border-2 bg-background p-1 transition-colors ${isSelected ? 'border-primary' : 'border-transparent hover:border-primary/40'}`}><img src={imageUrl} alt={`${product.name} – image ${image.displayOrder}`} className="h-28 w-full rounded-lg object-cover" /></button>;
                })}
              </div>
            ) : <p className="mt-4 rounded-xl border border-dashed border-border bg-background px-4 py-8 text-center text-sm text-muted-foreground">Aucune image supplémentaire dans le catalogue.</p>}
          </section>
        </div>
      )}
    </Modal>
...
function ProductFormModal({ isOpen, product, storeName, definitions, saving, onClose, onSubmit, onDeleteDocument, onDownloadDocument }: {
  isOpen: boolean;
  ...
  onClose: () => void;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
  onDeleteDocument: (productId: number, documentId: number) => void;
  onDownloadDocument: (productId: number, documentId: number, fileName: string) => void;
}) {
  const [pendingCatalogFiles, setPendingCatalogFiles] = useState<File[]>([]);
  const [catalogPreviews, setCatalogPreviews] = useState<string[]>([]);
  const [catalogImageIdsToDelete, setCatalogImageIdsToDelete] = useState<number[]>([]);
  const catalogInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    const previews = pendingCatalogFiles.map((file) => URL.createObjectURL(file));
    setCatalogPreviews(previews);
    return () => previews.forEach((url) => URL.revokeObjectURL(url));
  }, [pendingCatalogFiles]);
  useEffect(() => {
    if (!isOpen) {
      setPendingCatalogFiles([]);
      setCatalogImageIdsToDelete([]);
    }
  }, [isOpen, product?.id]);

  const syncPendingCatalogFiles = (files: File[]) => {
    const transfer = new DataTransfer();
    files.forEach((file) => transfer.items.add(file));
    if (catalogInputRef.current) catalogInputRef.current.files = transfer.files;
  };
  const visibleCatalogImages = (product?.catalogImages || []).filter((image) => !catalogImageIdsToDelete.includes(image.id));
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="rounded-2xl border border-border bg-muted/10 p-4">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="font-semibold text-foreground">Catalogue d’images</h3>
              <p className="mt-1 text-sm text-muted-foreground">Ajoutez jusqu’à 8 images supplémentaires. Elles sont indépendantes de l’image vitrine.</p>
            </div>
            <span className="text-xs font-medium text-muted-foreground">{visibleCatalogImages.length + catalogPreviews.length}/8 images</span>
          </div>
          <label className="mt-4 flex min-h-24 cursor-pointer items-center justify-center gap-3 rounded-xl border border-dashed border-primary/35 bg-background px-4 text-sm font-medium text-primary transition-colors hover:bg-primary/5">
            <Upload className="h-5 w-5" />
            Ajouter des images au catalogue
            <input ref={catalogInputRef} name="catalogImages" type="file" accept="image/*" multiple className="hidden" onChange={(event) => {
              const input = event.target;
              const newFiles = Array.from(input.files || []);
              setPendingCatalogFiles((current) => {
                const remainingSlots = Math.max(0, 8 - visibleCatalogImages.length);
                const nextFiles = [...current, ...newFiles].slice(0, remainingSlots);
                if (current.length + newFiles.length > remainingSlots) toast.error(`Vous pouvez ajouter encore ${Math.max(0, remainingSlots - current.length)} image(s) au maximum.`);
                const transfer = new DataTransfer(); nextFiles.forEach((file) => transfer.items.add(file)); input.files = transfer.files;
                return nextFiles;
              });
            }} />
          </label>
          {catalogImageIdsToDelete.map((imageId) => <input key={imageId} type="hidden" name="catalogImageIdsToDelete" value={imageId} />)}
          {(visibleCatalogImages.length || catalogPreviews.length > 0) && (
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {visibleCatalogImages.map((image) => (
                <div key={image.id} className="group relative overflow-hidden rounded-xl border border-border bg-background">
                  <img src={resolveApiUrl(image.imageUrl)} alt={`Image catalogue ${image.displayOrder}`} className="h-24 w-full object-cover" />
                  <button type="button" aria-label="Supprimer cette image du catalogue" onClick={() => setCatalogImageIdsToDelete((current) => [...current, image.id])} className="absolute right-2 top-2 rounded-md bg-background/90 p-1.5 text-destructive shadow-sm transition-colors hover:bg-destructive hover:text-white"><Trash2 className="h-4 w-4" /></button>
                </div>
              ))}
              {catalogPreviews.map((url, index) => <div key={url} className="group relative overflow-hidden rounded-xl border border-primary/30"><img src={url} alt={`Nouvelle image catalogue ${index + 1}`} className="h-24 w-full object-cover" /><button type="button" aria-label="Retirer cette nouvelle image" onClick={() => setPendingCatalogFiles((current) => { const nextFiles = current.filter((_, fileIndex) => fileIndex !== index); syncPendingCatalogFiles(nextFiles); return nextFiles; })} className="absolute right-2 top-2 rounded-md bg-background/90 p-1.5 text-destructive shadow-sm transition-colors hover:bg-destructive hover:text-white"><Trash2 className="h-4 w-4" /></button></div>)}
            </div>
          )}
          {product && <p className="mt-3 text-xs text-muted-foreground">Les ajouts et suppressions seront appliqués lorsque vous enregistrerez les modifications.</p>}
        </div>
        ...
        {definitions.length > 0 && (
          <div className="rounded-2xl border border-border p-4">
            <h3 className="font-semibold text-foreground">Caractéristiques {storeProductFamilyLabel[getStoreProductFamily(storeName || product?.storeName)]}</h3>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {definitions.map((definition) => (
                <FormField key={definition.id} label={definition.name}>
                  {(() => {
                    const input = getStoreParameterInput(storeName || product?.storeName, definition.name);
                    return <input name={`param-${definition.id}`} type={input.type} min={input.min} step={input.step} defaultValue={product?.parameterValues.find((value) => value.definitionId === definition.id)?.value} className="h-11 w-full rounded-lg border border-input bg-input-background px-3 outline-none focus:ring-2 focus:ring-ring" />;
                  })()}
                </FormField>
              ))}
            </div>
        ...
        <div className="rounded-2xl border border-border p-4">
          <h3 className="font-semibold text-foreground">Documents produit</h3>
          <p className="mt-1 text-sm text-muted-foreground">Ajoutez des fiches techniques, brochures, garanties ou documents de conformité. Choisissez s&apos;ils seront visibles publiquement.</p>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <FormField label="Type de document"><input name="documentType" defaultValue="Document produit" className="h-11 w-full rounded-lg border border-input bg-input-background px-3 outline-none focus:ring-2 focus:ring-ring" /></FormField>
            <FormField label="Fichiers"><input name="documents" type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,image/*" className="block h-11 w-full rounded-lg border border-input bg-input-background px-3 py-2 text-sm" /></FormField>
          </div>
          <label className="mt-3 flex items-center gap-2 text-sm text-foreground"><input name="publicDocument" type="checkbox" />Rendre ces documents visibles sur la marketplace</label>
          {product?.documents?.length ? <div className="mt-4 space-y-2">{product.documents.map((document) => <div key={document.id} className="flex items-center justify-between gap-3 rounded-lg bg-muted/40 px-3 py-2 text-sm"><span className="min-w-0 truncate">{document.documentType} · {document.fileName}</span><div className="flex items-center gap-2"><Badge variant={document.publicDocument ? 'success' : 'secondary'}>{document.publicDocument ? 'Public' : 'Privé'}</Badge><button type="button" aria-label={`Télécharger ${document.fileName}`} onClick={() => onDownloadDocument(product.id, document.id, document.fileName)} className="rounded-md p-1 text-primary hover:bg-primary/10"><Download className="h-4 w-4" /></button><button type="button" aria-label={`Supprimer ${document.fileName}`} onClick={() => onDeleteDocument(product.id, document.id)} className="rounded-md p-1 text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" /></button></div></div>)}</div> : null}
        </div>
