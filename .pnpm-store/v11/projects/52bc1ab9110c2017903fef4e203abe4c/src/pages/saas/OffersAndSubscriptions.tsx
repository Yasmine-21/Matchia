...
const formatAmount = (value?: number | null, _currency?: string | null) => {
...
export function OffersAndSubscriptions() {
  ...
  const confirmRenewal = async () => {
    ...
    try {
      setIsRenewing(true);
      const response = await apiClient.post<SubscriptionRenewalResponse>(`/api/payments/${renewalSubscription.paymentId}/renewal`);
  ...
  return (
    <div className="space-y-6">
                ...
                {subscriptions.map((subscription) => {
                  ...
                  const statusVariant =
                    subscription.status === 'PENDING_PAYMENT'
                      ? 'default'
                      : subscription.status === 'PENDING_RENEWAL' || isExpiringSoon
                          ? 'warning'
                  ...
                  const displayStatusLabel = subscription.renewalPending
                    ? 'Renouvellement en attente'
                    : isExpiringSoon ? 'Bientôt expiré' : statusLabel;
                  const canRenew = Boolean(subscription.paymentId) && (subscription.renewalEligible || subscription.renewalPending);
                  ...
                  return (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge variant={statusVariant}>{displayStatusLabel}</Badge>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            className={!canRenew ? 'cursor-not-allowed opacity-50' : ''}
                          >
                            {subscription.renewalPending ? 'Reprendre le paiement' : 'Renouveler'}
                          </Button>
                        </div>
                      </td>
                    </tr>
      ...
      <Modal
        ...
        title={renewalSubscription?.renewalPending ? 'Reprendre le paiement' : 'Renouveler l’abonnement'}
        size="lg"
      >
        {renewalSubscription && (() => {
          ...
          return (
            <div className="space-y-5">
              ...
              <p className="text-sm text-muted-foreground">
                {renewalSubscription.renewalPending
                  ? 'Un paiement de renouvellement est déjà en attente. Vous allez être redirigé vers le lien de paiement existant.'
                  : 'Un lien de paiement sécurisé sera ouvert pour l’administrateur de la banque. Après confirmation du paiement, l’abonnement sera prolongé d’une année supplémentaire.'}
              </p>
              <div className="flex justify-end gap-3">
                <Button variant="outline" disabled={isRenewing} onClick={() => setIsRenewalOpen(false)}>Annuler</Button>
                <Button disabled={isRenewing} onClick={confirmRenewal}>
                  {isRenewing ? 'Ouverture...' : renewalSubscription.renewalPending ? 'Reprendre le paiement' : 'Renouveler'}
                </Button>
              </div>
            </div>
          );
