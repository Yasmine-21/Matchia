...
export function SaaSStoresModules() {
  ...
  const [parameterToDelete, setParameterToDelete] = useState<{ moduleStoreId: number; parameterId: number } | null>(null);
  const [isDeletingParameter, setIsDeletingParameter] = useState(false);
  ...
  const confirmDeleteStore = async () => {
    ...
    try {
      ...
      const message = typeof responseData === 'string'
        ? responseData
        : responseData?.detail || responseData?.message || 'Erreur lors de la suppression du store';
      alert(message);
    } finally {
  ...
  const handleDeleteParam = (moduleStoreId: number, paramId: number) => {
    setParameterToDelete({ moduleStoreId, parameterId: paramId });
  };

  const confirmDeleteParam = async () => {
    if (!parameterToDelete) return;

    setIsDeletingParameter(true);
    try {
      const response = await moduleService.deleteModuleStoreParameter(parameterToDelete.parameterId);
      setAssignedModules((prev) =>
        prev.map((item) => {
          if (item.id === parameterToDelete.moduleStoreId) {
            return {
              ...item,
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        isOpen={parameterToDelete !== null}
        onClose={() => {
          if (!isDeletingParameter) {
            setParameterToDelete(null);
          }
        }}
      ...
      >
        <div className="space-y-5 text-center">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400">
            <AlertTriangle className="h-7 w-7" />
          </div>
          <div>
            <p className="text-base font-semibold text-gray-900 dark:text-white">Confirmer la suppression</p>
            <p className="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">
              Ce paramètre sera supprimé définitivement de ce module.
            </p>
          </div>
          <div className="flex flex-col-reverse justify-center gap-3 pt-1 sm:flex-row">
            <Button variant="outline" onClick={() => setParameterToDelete(null)} disabled={isDeletingParameter}>
              Annuler
            </Button>
            <Button variant="danger" onClick={confirmDeleteParam} disabled={isDeletingParameter} className="bg-red-500 hover:bg-red-600">
              {isDeletingParameter ? <><Loader2 className="h-4 w-4 animate-spin" /> Suppression...</> : 'Supprimer'}
            </Button>
          </div>
        </div>
      </Modal>
