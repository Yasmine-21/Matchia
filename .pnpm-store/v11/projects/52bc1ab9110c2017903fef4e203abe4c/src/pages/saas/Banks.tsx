...
export function SaaSBanks() {
  ...
  const filteredBanks = useMemo(() => banksList.filter((bank) => {
    ...
    return matchesSearch && (statusFilter === 'all' || bank.status === statusFilter);
  }), [banksList, searchTerm, statusFilter]);
      ...
      <Card className="mb-6">
        <CardContent className="pt-6">
          <div className="flex flex-col gap-3 md:flex-row">
            ...
            <label className="relative flex items-center md:w-52">
              <Filter className="pointer-events-none absolute left-3 h-4 w-4 text-muted-foreground" aria-hidden="true" />
              <select
                aria-label="Filtrer les banques par statut"
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as BankStatus | 'all')}
                className="h-10 w-full appearance-none rounded-md border border-input bg-background py-2 pl-9 pr-3 text-sm outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20"
              >
