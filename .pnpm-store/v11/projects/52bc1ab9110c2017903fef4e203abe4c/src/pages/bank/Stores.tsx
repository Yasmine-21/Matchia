...
import type {
  MarketplaceStoreBannerImageDto,
  ModuleAssignment,
...
export function BankStores() {
  ...
  const [bannerImages, setBannerImages] = useState<MarketplaceStoreBannerImageDto[]>([]);
  const [bannerSlot, setBannerSlot] = useState<number | null>(null);
  ...
  const visibleModulesByStore = useMemo(() => {
    return stores.reduce<Record<number, ModuleAssignment[]>>((acc, store) => {
      const storeId = getStoreNumericId(store);
      const configurationByModuleId = new Map(
        (modulesByStore[storeId] || []).map((assignment) => [assignment.module?.id, assignment]),
      );

      acc[storeId] = (store.modules || []).map((marketplaceModule, index) => {
        const moduleId = marketplaceModule.moduleId ?? marketplaceModule.id;
        const configuredAssignment = configurationByModuleId.get(moduleId);
        const isEnabled = marketplaceModule.enabled !== false && marketplaceModule.visible !== false;

        if (configuredAssignment) {
          return { ...configuredAssignment, actif: isEnabled };
        }
  ...
  const getAssignedModuleCountForStore = (store: { storeId?: number | null; id: number }) => {
    const marketplaceStore = stores.find((entry) => getStoreNumericId(entry) === getStoreNumericId(store));
    return (marketplaceStore?.modules || []).length;
  };
  ...
  const storeModules = useMemo(() => {
    ...
    return visibleModulesByStore[selectedStoreKeyNumber] || [];
  }, [selectedStoreKeyNumber, visibleModulesByStore]);
  ...
  useEffect(() => {
    if (!selectedStore || !isBannerFeatureActive) {
      setBannerImages([]);
      return;
    }
    let mounted = true;
    void bankTenantService.getStoreBanners(selectedStore.id)
      .then((response) => {
        if (mounted) setBannerImages(response.bannerImages || []);
      })
      .catch(() => {
        if (mounted) setBannerImages([]);
      });
    return () => { mounted = false; };
  }, [isBannerFeatureActive, selectedStore?.id]);
  ...
  const selectStoreBanner = (slot: number, file?: File) => {
    if (!file) return;
  ...
  const saveStoreBanner = async () => {
    if (!selectedStore || !bannerFile || bannerSlot === null) {
      toast.error('Sélectionnez une image avant de l’enregistrer.');
      return;
    }
    setIsSavingStoreBanner(true);
    try {
      const existingImage = bannerImages[bannerSlot];
      const response = existingImage?.id
        ? await bankTenantService.replaceStoreBannerImage(selectedStore.id, existingImage.id, bannerFile)
        : existingImage
          ? await bankTenantService.uploadStoreBanner(selectedStore.id, bannerFile).then(() => bankTenantService.getStoreBanners(selectedStore.id))
          : await bankTenantService.addStoreBanner(selectedStore.id, bannerFile);
      setBannerImages(response.bannerImages || []);
  ...
  const removeStoreBanner = async (slot: number) => {
    const image = bannerImages[slot];
    if (!selectedStore || !image) return;
    setIsSavingStoreBanner(true);
    try {
      if (image.id) {
        await bankTenantService.deleteStoreBannerImage(selectedStore.id, image.id);
      } else {
        await bankTenantService.deleteLegacyStoreBanner(selectedStore.id);
      }
      const response = await bankTenantService.getStoreBanners(selectedStore.id);
      setBannerImages(response.bannerImages || []);
      toast.success('Bannière supprimée.');
      refresh();
    } catch (removeError) {
      console.error('Failed to remove store banner:', removeError);
      toast.error('Impossible de supprimer la bannière du store.');
    } finally {
      setIsSavingStoreBanner(false);
    }
  };
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {stores.map((store) => {
                  ...
                  const modulesCount = getAssignedModuleCountForStore(store);
                ...
                {false && isBannerFeatureActive && (
                  <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
                    ...
                    <input
                      ...
                      className="sr-only"
                      onChange={(event) => {
                        if (bannerSlot !== null) selectStoreBanner(bannerSlot, event.target.files?.[0]);
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                      {[0, 1, 2].map((slot) => {
                        const image = bannerImages[slot];
                        const preview = bannerSlot === slot && bannerPreviewUrl
                          ? bannerPreviewUrl
                          : resolveApiUrl(image?.imageUrl || (slot === 0 ? selectedStoreBannerUrl : ''));
                        return (
                          <div key={slot} className="rounded-xl border border-border bg-background p-3">
                            <div className="mb-2 flex items-center justify-between gap-2 text-sm font-semibold"><span>Image {slot + 1}</span>{image && <span className="text-xs font-normal text-muted-foreground">Position {slot + 1}</span>}</div>
                            <div className={`${BANNER_FRAME_CLASS} rounded-lg border border-border bg-muted`}>
                              {preview ? <img src={preview} alt={`Bannière ${slot + 1}`} className={BANNER_IMAGE_CLASS} /> : <div className="flex h-full flex-col items-center justify-center gap-2 text-xs text-muted-foreground"><FileImage className="h-6 w-6" />Aucune image</div>}
                            </div>
                            <div className="mt-3 flex gap-2">
                              <Button type="button" size="sm" variant="outline" className="flex-1" icon={<Upload className="h-4 w-4" />} disabled={isSelectedStoreInactive || (!image && bannerImages.length >= 3)} onClick={() => { setBannerSlot(slot); bannerInputRef.current?.click(); }}>{image ? 'Remplacer' : 'Ajouter'}</Button>
                              {image && <Button type="button" size="sm" variant="outline" aria-label={`Supprimer l'image ${slot + 1}`} disabled={isSavingStoreBanner || isSelectedStoreInactive} onClick={() => void removeStoreBanner(slot)}><Trash2 className="h-4 w-4" /></Button>}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      <Button type="button" loading={isSavingStoreBanner} disabled={!bannerFile || bannerSlot === null || isSelectedStoreInactive} onClick={saveStoreBanner}>Enregistrer l’image sélectionnée</Button>
                      <p className="text-xs text-muted-foreground">{bannerFile?.name ?? `Jusqu’à 3 images · PNG, JPG, WEBP ou GIF (5 Mo max.) · recommandé : ${BANNER_RECOMMENDED_DIMENSIONS}`}</p>
                    </div>
                    <div className="hidden">
                      <div className={`${BANNER_FRAME_CLASS} rounded-xl border border-border bg-background`}>
                      ...
                      <div className="flex flex-col gap-2 lg:w-52">
                        <input
                          ...
                          accept="image/png,image/jpeg,image/webp,image/gif"
                          className="sr-only"
                          onChange={(event) => selectStoreBanner(0, event.target.files?.[0])}
                        />
                        <Button
                          type="button"
                          variant="outline"
                        ...
                        <p className="text-xs text-muted-foreground">
                          {bannerFile?.name ?? `PNG, JPG, WEBP ou GIF (5 Mo max.) · recommandé : ${BANNER_RECOMMENDED_DIMENSIONS}`}
                        </p>
                      </div>
                    </div>
                  </div>
