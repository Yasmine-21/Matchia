    ...
    <div className="space-y-6">
                  ...
                  <section key={storeId ?? 'marketplace'} className="space-y-4 rounded-3xl border border-border bg-muted/20 p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-semibold" style={{ borderColor: `${storeColor}33`, backgroundColor: `${storeColor}10`, color: storeColor }}>
                          <StoreIcon className="h-4 w-4 shrink-0" style={{ color: storeColor }} />
                          <span className="truncate">{storeLabel}</span>
                        </div>
                        <p className="mt-2 text-sm text-muted-foreground">{storeDescription}</p>
                      </div>
                      <div className="rounded-full bg-background px-3 py-1 text-xs font-medium text-muted-foreground">{storeContents.length} contenu{storeContents.length > 1 ? 's' : ''}</div>
                    </div>
                    <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                      {storeContents.map((content) => {
                    const imageUrl = resolveApiUrl(content.imageUrl);
                    return (
                      <article key={`${content.source}-${content.id}`} className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-transform hover:-translate-y-1 hover:shadow-lg">
                        <div className="relative h-48 bg-gradient-to-br from-slate-900 via-slate-800 to-orange-500">
                          {imageUrl ? <img src={imageUrl} alt={content.title} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-white/80"><ImageIcon className="h-12 w-12" /></div>}
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/65 to-transparent p-4"><Badge variant={content.status === 'active' ? 'success' : 'warning'}>{statusLabel[content.status]}</Badge></div>
                        </div>
                        <div className="flex flex-1 flex-col gap-4 p-5">
                          <div className="space-y-2"><h3 className="text-lg font-semibold text-slate-900">{content.title}</h3><p className="text-sm text-muted-foreground">{truncate(content.description)}</p></div>
                          {content.source === 'standard' && (
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><div className="flex items-center justify-between gap-3"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Visibilité marketplace</div><div className="mt-1 text-sm font-medium text-slate-900">{content.visibleInMarketplace === false ? 'Masqué' : 'Visible'}</div></div><Button variant={content.visibleInMarketplace === false ? 'success' : 'outline'} size="sm" className="shrink-0" icon={visibilityUpdatingContentId === content.id ? <Loader2 className="h-4 w-4 animate-spin" /> : content.visibleInMarketplace === false ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />} onClick={() => toggleStandardVisibility(content)} disabled={visibilityUpdatingContentId === content.id}>{content.visibleInMarketplace === false ? 'Afficher' : 'Masquer'}</Button></div></div>
                          )}
                          {content.source === 'marketplace' && (
                            <div className="mt-auto flex flex-col gap-3 sm:flex-row"><Button variant="outline" className="w-full" icon={<Pencil className="h-4 w-4" />} onClick={() => openEditModal(content)}>Modifier</Button><Button variant="danger" className="w-full" icon={deletingContentId === content.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} onClick={() => deleteContent(content)} disabled={deletingContentId === content.id}>Supprimer</Button></div>
                          )}
                        </div>
                      </article>
                    );
      ...
      >
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-border bg-slate-50/60 p-4 dark:bg-slate-900/30">
            <div className="mb-4 flex items-center gap-3">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary">
                <StoreIcon className="h-4 w-4" />
              </span>
              <div>
                <h3 className="text-sm font-semibold text-foreground">Informations du contenu</h3>
                <p className="text-xs text-muted-foreground">Choisissez le store et définissez la visibilité.</p>
              </div>
            </div>
          ...
          <section className="space-y-4">
            <Input
              ...
              placeholder="Ex. Découvrez nos offres de financement"
            />
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Décrivez votre contenu et les informations importantes pour vos clients..."
              />
            </div>
          </section>

          <section className="grid gap-4 rounded-2xl border border-border bg-card p-4 md:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-2">
              <label className="block text-sm font-medium text-foreground">Image principale</label>
              <p className="text-xs text-muted-foreground">PNG, JPG ou WEBP. L&apos;image sera affichée dans la marketplace.</p>
              <Input
                type="file"
                accept="image/*"
                onChange={(event) => handleImageChange(event.target.files?.[0] || null)}
                className="cursor-pointer file:mr-3 file:rounded-md file:border-0 file:bg-primary/10 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-primary hover:file:bg-primary/15"
              />
            </div>

            <div className="rounded-xl border border-dashed border-border bg-muted/30 p-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <ImageIcon className="h-4 w-4 text-primary" /> Aperçu
              </div>
              <div className="mt-3 flex h-40 overflow-hidden rounded-lg border border-border bg-slate-100 dark:bg-slate-900">
                {imagePreview ? (
