...
class ProductControllerTest {
    ...
    @Test
    void updatesAndDeletesJsonProducts() {
        ProductService service = mock(ProductService.class);
        ProductController controller = new ProductController(service);
    ...
    @Test
    void buildsMultipartUpdateRequestsIncludingPriceAndParameters() throws Exception {
        ProductService service = mock(ProductService.class);
        ProductController controller = new ProductController(service);
        ProductDto product = new ProductDto();
        ...
        assertThat(controller.updateMultipart(4L, 2L, 3L, "Loan", "desc", "12.50", image,
                "[{\"parameterDefinitionId\":5,\"value\":\"yes\"}]").getStatusCode().value()).isEqualTo(200);
        verify(service).update(eq(4L), argThat(request -> request.getPrice().toString().equals("12.50")
    ...
    void mapsMultipartInputAndStorageErrors() throws Exception {
        ...
        ProductController controller = new ProductController(service);
        MultipartFile image = mock(MultipartFile.class);
        assertThat(controller.updateMultipart(1L, 1L, 2L, "name", null, "bad-price", image, null)
