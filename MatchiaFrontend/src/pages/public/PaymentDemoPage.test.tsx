...
describe('PaymentDemoPage', () => {
  it('shows order information and rejects an invalid amount before calling the backend', async () => {
    render(<MemoryRouter initialEntries={['/payment-demo?bank=Atlas&plan=Premium&amount=0&currency=usd']}><PaymentDemoPage /></MemoryRouter>);

    expect(screen.getByText('Premium')).toBeInTheDocument();
    expect(screen.getByText('Atlas')).toBeInTheDocument();
    expect(screen.getByText('Paiement Matchia - TND')).toBeInTheDocument();
