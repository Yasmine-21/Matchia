...
const labels: Record<PartnershipContract['status'], string> = { DRAFT: 'Brouillon', PENDING_ACCEPTANCE: "A valider", ACTIVE: 'Actif', SUPERSEDED: 'Version remplacée', EXPIRED: 'Expire', TERMINATED: 'Resilie', CANCELLED: 'Refuse' };
...
export function DealerContractsPanel({ contracts, onChanged }: { contracts: PartnershipContract[]; onChanged: () => Promise<void> }) {
  ...
  return <>
    <div className="grid gap-5 xl:grid-cols-2">{contracts.map((contract) => <Card key={contract.id} className="flex h-full flex-col p-6">
      <div className="flex items-start justify-between gap-4"><div><div className="text-xs uppercase tracking-widest text-primary">{contract.contractNumber}</div><h3 className="mt-2 text-xl font-semibold">Partenariat avec {contract.bankName}</h3><p className="text-sm text-muted-foreground">{contract.storeName} · Version {contract.versionNumber}</p></div><Badge variant={contract.status === 'ACTIVE' ? 'success' : contract.status === 'CANCELLED' ? 'danger' : 'warning'}>{labels[contract.status]}</Badge></div>
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm"><Info label="Commission" value={formatCommission(contract)} /><Info label="Debut" value={contract.startDate} /><Info label="Fin" value={contract.endDate} /></div>
      <div className="mt-auto flex gap-3 pt-5"><Button variant="outline" icon={<Eye className="h-4 w-4" />} onClick={() => setSelected(contract)}>Aperçu</Button>{contract.status === 'PENDING_ACCEPTANCE' && !contract.dealerAcceptedAt && <><Button icon={<CheckCircle2 className="h-4 w-4" />} loading={busy === `accept-${contract.id}`} onClick={() => void accept(contract)}>Accepter</Button><Button variant="danger" icon={<XCircle className="h-4 w-4" />} onClick={() => setRejecting(contract)}>Refuser</Button></>}</div>
    </Card>)}</div>
    <ContractPreviewModal contract={selected} side="dealer" onClose={() => setSelected(null)} />
